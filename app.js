...
function factsBlockHTML(view) {
  ...
  const step = withMaterial(view, () => lessons ? doneLessons().size : curStats().days);
  const next = list.find(f => !f.open);

  return `
    ${next
      ? `<div class="facts-note">${lessons ? "Пройдено уроков" : "Занятий с материалом"}: <b>${step}</b>. Следующая карточка — после <b>${next.need}</b>. Когда материал будет пройден, откроются все.</div>`
    ...
    <div class="facts-grid">
      ${list.map(f => f.open
        ...
        : `<div class="fcard locked">
             <span class="fi">🔒</span>
             <span class="ft">После ${f.need} ${lessons
