...
const ACH_SCREWTAPE = [
  ...
  { id: "l1", icon: "🏠", name: "Обычная жизнь", hint: "Дочитать письмо 1-е", test: s => s.page >= 26 },
  { id: "l2", icon: "⛪", name: "Соседи по скамье", hint: "Дочитать письмо 2-е", test: s => s.page >= 31 },
  { id: "l3", icon: "🍽️", name: "Домашние мелочи", hint: "Дочитать письмо 3-е", test: s => s.page >= 36 },
  { id: "l4", icon: "🙏", name: "Молитва без слов", hint: "Дочитать письмо 4-е", test: s => s.page >= 41 },
  { id: "l5", icon: "💣", name: "Война не так выгодна", hint: "Дочитать письмо 5-е", test: s => s.page >= 46 },
  { id: "l6", icon: "🌫️", name: "Тревога о будущем", hint: "Дочитать письмо 6-е", test: s => s.page >= 51 },
  { id: "l7", icon: "🚩", name: "Партия важнее веры", hint: "Дочитать письмо 7-е", test: s => s.page >= 56 },
  { id: "l8", icon: "🌊", name: "Приливы и отливы", hint: "Дочитать письмо 8-е", test: s => s.page >= 61 },
  { id: "l9", icon: "🏜️", name: "Спад как удобный момент", hint: "Дочитать письмо 9-е", test: s => s.page >= 67 },
  { id: "l10", icon: "👥", name: "Новые знакомые", hint: "Дочитать письмо 10-е", test: s => s.page >= 72 },
  { id: "l11", icon: "🎭", name: "Пять видов смеха", hint: "Дочитать письмо 11-е", test: s => s.page >= 77 },
  { id: "l12", icon: "🛤️", name: "Дорога незаметных шагов", hint: "Дочитать письмо 12-е", test: s => s.page >= 83 },
  { id: "l13", icon: "🍎", name: "Настоящее удовольствие", hint: "Дочитать письмо 13-е", test: s => s.page >= 89 },
  { id: "l14", icon: "🪞", name: "Гордость своим смирением", hint: "Дочитать письмо 14-е", test: s => s.page >= 94 },
  { id: "l15", icon: "⏳", name: "Настоящее и будущее", hint: "Дочитать письмо 15-е", test: s => s.page >= 99 },
  { id: "l16", icon: "🚶", name: "Церковный туризм", hint: "Дочитать письмо 16-е", test: s => s.page >= 104 },
  { id: "l17", icon: "🍷", name: "Гурманство наоборот", hint: "Дочитать письмо 17-е", test: s => s.page >= 110 },
  { id: "l18", icon: "💍", name: "Любовь и брак", hint: "Дочитать письмо 18-е", test: s => s.page >= 115 },
  { id: "l19", icon: "❤️", name: "Что такое любовь", hint: "Дочитать письмо 19-е", test: s => s.page >= 120 },
  { id: "l20", icon: "🕶️", name: "Мода на вкусы", hint: "Дочитать письмо 20-е", test: s => s.page >= 126 },
  { id: "l21", icon: "⏱️", name: "Моё время, моя жизнь", hint: "Дочитать письмо 21-е", test: s => s.page >= 132 },
  { id: "l22", icon: "🎉", name: "Чужая радость", hint: "Дочитать письмо 22-е", test: s => s.page >= 137 },
  { id: "l23", icon: "📜", name: "Исторический Иисус", hint: "Дочитать письмо 23-е", test: s => s.page >= 143 },
  { id: "l24", icon: "🎩", name: "Духовный снобизм", hint: "Дочитать письмо 24-е", test: s => s.page >= 149 },
  { id: "l25", icon: "🔮", name: "Мода на новизну", hint: "Дочитать письмо 25-е", test: s => s.page >= 155 },
  { id: "l26", icon: "🤐", name: "Уступчивость как оружие", hint: "Дочитать письмо 26-е", test: s => s.page >= 161 },
  { id: "l27", icon: "🕰️", name: "Молитва и время", hint: "Дочитать письмо 27-е", test: s => s.page >= 167 },
  { id: "l28", icon: "🛋️", name: "Долгая безопасная жизнь", hint: "Дочитать письмо 28-е", test: s => s.page >= 173 },
  { id: "l29", icon: "🦁", name: "Мужество и страх", hint: "Дочитать письмо 29-е", test: s => s.page >= 179 },
  { id: "l30", icon: "🥱", name: "Усталость", hint: "Дочитать письмо 30-е", test: s => s.page >= 185 },
  { id: "l31", icon: "📮", name: "Последнее письмо", hint: "Дочитать письмо 31-е", test: s => s.page >= 191 },
...
const WORDS_SCREWTAPE = {
  ...
  l1: "Письмо 1-е · Первый совет: держать человека в гуще привычных дел, подальше от рассуждений — «настоящая жизнь» звучит убедительнее любых доводов",
  l2: "Письмо 2-е · О том, как разочарование в прихожанах бьёт точнее, чем спор о вере",
  l3: "Письмо 3-е · О раздражении, которое копится в быту и в разговорах с самыми близкими",
  l4: "Письмо 4-е · О том, как испортить молитву — усложнив её или сведя к ощущениям",
  l5: "Письмо 5-е · Неожиданный поворот: старший бес объясняет, почему беда сама по себе не работает на них",
  l6: "Письмо 6-е · О страхе перед неизвестным и о том, куда девать внимание",
  l7: "Письмо 7-е · О политических крайностях и о том, как дело подменяет собой человека",
  l8: "Письмо 8-е · О законе чередования: подъёмы сменяются спадами, и это нормально",
  l9: "Письмо 9-е · О том, чем опасны периоды сухости и усталости",
  l10: "Письмо 10-е · О компании, под которую незаметно подстраиваешься",
  l11: "Письмо 11-е · О юморе: где он живой, а где становится способом ничего не чувствовать",
  l12: "Письмо 12-е · Самая известная мысль книги: вниз ведёт пологая дорога без поворотов и вех",
  l13: "Письмо 13-е · О вкусе к подлинному — и почему он мешает искушению",
  l14: "Письмо 14-е · О самой ловкой подмене: гордиться тем, что ты скромен",
  l15: "Письмо 15-е · О жизни в тревоге о завтрашнем дне вместо сегодняшнего",
  l16: "Письмо 16-е · О поиске «правильного» прихода вместо того, чтобы просто быть в своём",
  l17: "Письмо 17-е · О чревоугодии изысканности — когда придирчивость выдают за тонкий вкус",
  l18: "Письмо 18-е · О подмене понятий там, где речь о близости",
  l19: "Письмо 19-е · О путанице между влюблённостью, привязанностью и любовью",
  l20: "Письмо 20-е · О навязанных образах желаемого",
  l21: "Письмо 21-е · О чувстве собственности — на время, на планы, на себя",
  l22: "Письмо 22-е · О том, что делает бесов совершенно беспомощными",
  l23: "Письмо 23-е · О вере, превращённой в идею и в политическую позицию",
  l24: "Письмо 24-е · О чувстве избранности и о круге «своих»",
  l25: "Письмо 25-е · О тяге к усложнённым версиям простого",
  l26: "Письмо 26-е · О мнимой деликатности, за которой копится обида",
  l27: "Письмо 27-е · О том, как сбить человека с молитвы учёными доводами",
  l28: "Письмо 28-е · О том, почему бесам выгоднее длинная спокойная жизнь",
  l29: "Письмо 29-е · О трусости как главной цели — и о том, что ей противостоит",
  l30: "Письмо 30-е · О измотанности, которая делает человека податливым",
  l31: "Письмо 31-е · Переписка окончена — и заканчивается она совсем не так, как рассчитывал её автор",
...
const BOOK_ACH = { "snow-1": ACH_BOOK, odyssey: ACH_ODYSSEY, tesson: ACH_TESSON, screwtape: ACH_SCREWTAPE };
const BOOK_WORDS = { "snow-1": WORDS_BOOK, odyssey: WORDS_ODYSSEY, tesson: WORDS_TESSON, screwtape: WORDS_SCREWTAPE };
  ...
  if (item.track === "book") {
    ...
    return `
      <div class="cover book ${esc(b.tone || "sea")}">
        ...
        ${b.art === "wave" ? SEA_ART : b.art === "pine" ? PINE_ART : b.art === "quill" ? QUILL_ART : `<div class="cv-mark">🦔</div>`}
        <div>
